...
class pdfView( View ):
    def get( self, request, *args, **kwargs ):
        try:
            return FileResponse( open( 'E:/study materials/algorithms/Dsa.pdf', 'rb' ),
...
class QuizListView(ListView):
    ...
    def get_queryset(self,*args,**kwargs):
        ...
        #ids = map(int, self.request.user.user.Class.split(","))
        #return Medicament.objects.filter(id__in=ids)
        obj = Category.objects.filter(category=self.request.user.user.Class)
        return queryset.filter(category=obj[0], draft=False)
...
class QuizTake(FormView):
    ...
    def final_result_user(self):
        resul = EResources.objects.get(subject__icontains=self.quiz, Class='EIGHT')
        print(resul.subject)
        print(self.quiz)
        results = {
            'quiz': self.quiz,
